...
def addContentData_w(*data) :
    conn = connection.get_connection()

    sql = '''
        insert into web_service_table
        (web_content_subject, web_content_writer_idx, web_content_date
        ,web_content_text, web_content_file, web_content_status, web_content_board_idx
        , web_view,  web_likes)
        values(%s, %s, curdate(), %s, %s, 1, 3, 0, 0 )
    '''
...
def get_paging_info(board_idx, page) :
    ...
    sql = '''
        ...
        where web_content_status = 1 and web_content_board_idx = %s
...
def getContentlist(board_idx, page):
    ...
    sql = '''
        select w.web_content_idx, w.web_content_subject, w.web_content_date,w.web_view, w.web_likes
               , u.user_id
        from web_service_table w, user_table u
        where w.web_content_writer_idx = u.user_idx
              and w.web_content_board_idx = %s
              and w.web_content_status = 1
        order by w.web_content_idx desc
        limit %s, 10
    '''
...
def readContent(board_idx, content_idx) :
    conn = connection.get_connection()
    sql = '''
        select w.web_content_subject, w.web_content_idx, w.web_content_date,
               w.web_content_text, w.web_content_file, w.web_view, w.web_likes,
               u.user_id, u.user_idx
        from web_service_table w, user_table u
        where w.web_content_board_idx = %s and w.web_content_status = 1
              and w.web_content_idx = %s
              and w.web_content_writer_idx = u.user_idx
    '''
...
def updateContent(board_subject, board_text, file_name, user_idx, content_idx):
    ...
    sql = '''
        ...
        set web_content_subject = %s,
                web_content_text = %s,
                web_content_file = %s
        where web_content_writer_idx = %s 
              and web_content_idx = %s
              and web_content_board_idx = 3
    '''
    cursor = conn.cursor()
...
def deleteContent(board_idx, content_idx):
    ...
    sql = '''
        update web_service_table
        set web_content_status = 0
        where web_content_idx = %s and web_content_writer_idx = %s
              and web_content_board_idx = %s and web_content_status = 1
    '''
...
def addView(board_idx, content_idx) :
    ...
    sql = '''
            update web_service_table
            set web_view = web_view + 1
            where web_content_idx = %s
              and web_content_board_idx = %s'''
